...
When the source is the student itself ($\pi=p_{\mathrm{ref}}$), $\pi^+=p_{\mathrm{ref}}^+$ and the mismatch term vanishes: $D_{\mathrm{KL}}(\pi^+\\,\Vert \\p_{\mathrm{ref}})=-\log\alpha_{\mathrm{ref}}$, the smallest KL attainable by *any* safe policy. A teacher source ($\pi=p_T$) incurs an additional cross-model gap $D_{\mathrm{KL}}(p_T^+\|p_{\mathrm{ref}}^+)\ge 0$.
